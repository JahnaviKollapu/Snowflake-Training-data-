...
select customer_email from bronze_hotel_booking where not
(customer_email like '%@%.%')
 or customer_email is null;

select total_amount from bronze_hotel_booking where try_to_number(total_amount)<0;
...
select 
    ...
    try_to_date(nullif(check_out_date,'')) as check_in_date,
    room_type,
    num_guests,
    ...
    try_to_date(check_out_date) is not null and try_to_date(check_in_date)<= try_to_date(check_out_date);
...
select 
    hotel_city,sum(total_amount) as total_revenue
from silver_hotel_bookings 
group by hotel_city 
